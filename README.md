...
- **transactions.address**: Array of addresses that should match the transaction's `to` field. If none of these match the `transaction.to` field, the transaction won't be included in the response. null or empty array means any address will pass.
