    ...
    async_stream::try_stream! {
        for dir_name in dir_names {
            ...
            if dir_name.range.from > block_num {
                return Err(Error::BlockNotFoundInS3(block_num));
            }
            ...
            let (blocks, txs, logs) = futures::future::try_join3(block_fut, tx_fut, log_fut).await?;
            ...
            for tx in txs {
                blocks[&tx.block_number.0].transactions.push(tx);
            }
...
async fn read_blocks(
    ...
    let key = format!("{}/block.parquet", dir_name);
...
async fn read_txs(
    ...
    let key = format!("{}/tx.parquet", dir_name);
    let file = read_file_from_s3(&key, s3_src_bucket, client).await?;
...
async fn read_logs(
    ...
    let key = format!("{}/log.parquet", dir_name);
    let file = read_file_from_s3(&key, s3_src_bucket, client).await?;
